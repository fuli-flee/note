...
它的主要作用是:
- 解耦程序模块, 降低程序耦合度
- 它可以降低不同模块的耦合度, 不需要直接引用或依赖于彼此的具体实现
...
![alt text](/Unity/图片/框架/基础框架/基础框架11-04_10-33-05.jpg)
...
在上面的代码中都没有实现触发事件时传递数据
我们目前触发事件，是通过执行委托中存储的函数来执行各系统对应的逻辑
那么需要传递参数，自然的联想到应该从委托入手

但是参数的类型可能有多种多样，我们可以采用 object 利用里式替换原则
父类容器装载子类对象的方式来传递参数
```CSharp
public class EventCenter : SingletonBaseManager<EventCenter>
{
...
根据上面的修改把其他相关代码稍微修修就行

***
## 4.4 参数类型自定义

根据以上的代码存在一个问题
使用object进行参数传递，当传递**值类型**数据时，会存在装箱拆箱，增加性能开销
...
1. 改委托的参数为泛型
```CSharp
public class EventCenter<T> : SingletonBaseManager<EventCenter<T>>
{
    private Dictionary<string, UnityAction<T>> eventDic = new Dictionary<string, UnityAction<T>>();
}
```

但是这样犯了一个严重的错误, 事件中心不唯一了

2. 声明一个抽象类进行装载
    首先我们要知道,事件中心类只能这么写,那该怎么去用泛型呢?

    ```CSharp
    public class EventCenter : SingletonBaseManager<EventCenter>
    ```
    声明一个抽象类, 然后用一个泛型类去包裹委托, 并继承抽象类
    - 为什么声明的是抽象类? 声明一个普通类不行吗?
    - 也行, 声明为抽象类的主要原因是: 抽象类不能被实例化

    那么就可以明确这是一个基类，不应该直接实例化
    ...
    ```
3. 将所有的函数都改为有泛型的,以`AddEventListener`为例
    ```CSharp
    ...
    ```
[文件详情请看这里](/Unity/阶段性文件/基础框架/事件中心/参数类型自定义/EventCenter.cs),除了上面的实现外,还增加了无参数的重载
...
无法正确监听或触发事件
总不能一直都用string吧, 调用的时候事件名写错是常有的事, 而且还不好查错
...
2. 然后把所有string相关传参的地方全部换成枚举
    这里以`AddEventListener`为例
```CSharp
public void AddEventListener(E_EventType eventName, UnityAction func)
```

至此, 事件中心模块编写完毕, [文件详情请看这里](/Unity/阶段性文件/基础框架/04事件中心/成品/)
***
# 五. 资源加载模块

坏了, 没学网络开发相关和AB包, 等我学完了再来写